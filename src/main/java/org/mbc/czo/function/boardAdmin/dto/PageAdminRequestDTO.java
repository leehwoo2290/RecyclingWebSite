...
public class PageAdminRequestDTO {
    ...
    private String link;  // 페이징 번호 차리시 문자열
    public String getLink(){
        if(link == null){
            StringBuilder builder = new StringBuilder();
            builder.append("페이지 = " + this.page);
            builder.append(" & 사이즈 = " + this.size);

            if(type != null && type.length() > 0){
                builder.append(" & 타입 = " + type);
            }
            if(keyword != null){
                try{
                    builder.append(" & 키워드 = " + URLEncoder.encode(keyword, "UTF-8"));
